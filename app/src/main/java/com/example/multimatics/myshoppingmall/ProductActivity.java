...
public class ProductActivity extends AppCompatActivity implements AdapterView.OnItemClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for (int i = 0; i < SampleData.buble.length; i++){
            mProduct = new Product();
            mProduct.setId(System.currentTimeMillis());
            mProduct.setName(SampleData.buble[i][0]);
            mProduct.setPrice(SampleData.buble[i][2]);
            mProduct.setImageUrl(SampleData.buble[i][1]);
